  ...
  if demo_mode == "📊 IMU实时监测":
      ...
      if len(st.session_state.imu_data['history']) > 1:
          ...
          fig_trend.add_trace(go.Scatter(x=df['time'], y=df['ax'], name='Ax', line=dict(color='red', dash='dash')), row=
  2, col=1)
          fig_trend.add_trace(go.Scatter(x=df['time'], y=df['ay'], name='Ay', line=dict(color='green', dash='dash')), ro
  w=2, col=1)
          fig_trend.add_trace(go.Scatter(x=df['time'], y=df['az'], name='Az', line=dict(color='blue', dash='dash')), row
  =2, col=1)
      ...
      with tab3:
          ...
          **实验步骤:**
          ...
          st.info("💡 **教学提示**: 在实际教学中，建议先使用本模拟系统演示原理，再让学生在真实F450平台上操作，避免炸机风
  险。")
